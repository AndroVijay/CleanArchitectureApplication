package com.example.cleanarchitectureapplication.data.mappers
...
import com.example.cleanarchitectureapplication.data.remote.response.CountryResponse
...
fun CountryResponse.toCountryEntity(): CountryEntity {
    return CountryEntity(
        ...
        currencies = currencies.entries.joinToString(",") {
            "${it.key}:${it.value.name} (${it.value.symbol})"
        },
        gini = if (gini.isNullOrEmpty()) { null } else { gini.entries.joinToString(", ") {
            "${it.key}: ${it.value}"
        } },
